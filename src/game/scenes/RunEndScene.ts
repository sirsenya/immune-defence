...
export class RunEndScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, 80, 'RUN ЗАВЕРШЁН', {
    ...
    for (let i = 0; i < lines.length; i++) {
      this.add.text(width / 2, 200 + i * 30, lines[i] ?? '', {
        fontFamily: '"Courier New", monospace',
        fontSize: '18px',
        color: '#d8c9a8',
      }).setOrigin(0.5);
    ...
    const verdict = totalCollected >= totalTarget
      ? '«ПЛАН ВЫПОЛНЕН. СЛАВА ОТДЕЛУ.»'
      : '«ПЛАН ПРОВАЛЕН. НО ОТДЕЛ ЕЩЁ СТОИТ.»';
    this.add.text(width / 2, height - 200, verdict, {
      fontFamily: '"Courier New", monospace',
      fontSize: '20px',
